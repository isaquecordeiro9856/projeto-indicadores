...
(function (angular) {
  ...
  app.factory('ApiService', function($q, $timeout, $http) {
    ...
    var ENDPOINT = pathname.indexOf('farmacia') !== -1 ? '/api/farmacia'
      : pathname.indexOf('centrocirurgico') !== -1 ? '/api/centrocirurgico'
      : pathname.indexOf('fisioterapia') !== -1 ? '/api/fisioterapia'
      : '/api/pempfrg';
    var DIMENSOES = {
      pempfrg: ['convenio','setor','medico_executor','procedimento','mes'],
    ...
    var INDICADORES = {
      pempfrg: ['todos','qtd_contas','qtd_procedimentos','valor_produzido','valor_medico'],
    ...
    function adaptarRequisicao(req) {
      var dominio = pathname.indexOf('farmacia') !== -1 ? 'farmacia'
        : pathname.indexOf('centrocirurgico') !== -1 ? 'centrocirurgico'
        : pathname.indexOf('fisioterapia') !== -1 ? 'fisioterapia' : 'pempfrg';
      var aliases = {
        farmacia: { estabelecimento: 'material', tipo_convenio: 'mes', convenio: 'antibiotico', qtd_contas: 'qtd_pacientes', qtd_procedimentos: 'qtd_prescricoes', qtd_matmed: 'qtd_materiais' },
        centrocirurgico: { procedimentos: 'procedimento', tipo_convenio: 'porte', tipo_atendimento: 'mes', medico_executor: 'medico' },
        fisioterapia: { procedimentos: 'procedimento', tipo_convenio: 'mes', medico_executor: 'medico', qtd_procedimentos: 'qtd_atendimentos' }
      }[dominio] || {};
      var dimensao = aliases[req.dimensao] || req.dimensao;
      if (DIMENSOES[dominio].indexOf(dimensao) === -1) dimensao = DIMENSOES[dominio][0];
      var indicador = aliases[req.indicador] || req.indicador;
      if (INDICADORES[dominio].indexOf(indicador) === -1) indicador = 'todos';
      return angular.extend({}, req, { dimensao: dimensao, indicador: indicador, dashboard: dominio === 'pempfrg' ? 'medico' : dominio });
    }
    function adaptarResposta(data) {
      var dominio = pathname.indexOf('farmacia') !== -1 ? 'farmacia'
        : pathname.indexOf('centrocirurgico') !== -1 ? 'centrocirurgico'
        : pathname.indexOf('fisioterapia') !== -1 ? 'fisioterapia' : 'pempfrg';
      if (dominio === 'pempfrg' || !data || !data.dados) return data;
      data.dados.forEach(function(item) {
        if (dominio === 'farmacia') {
          item.qtd_contas = item.qtd_pacientes || 0;
          item.qtd_procedimentos = item.qtd_prescricoes || 0;
          item.qtd_matmed = item.qtd_materiais || 0;
          item.qtd_contas_fmt = Number(item.qtd_contas).toLocaleString('pt-BR');
          item.qtd_procedimentos_fmt = Number(item.qtd_procedimentos).toLocaleString('pt-BR');
          item.qtd_matmed_fmt = Number(item.qtd_matmed).toLocaleString('pt-BR');
        }
        if (dominio === 'centrocirurgico') {
          item.qtd_contas = item.qtd_contas || 0;
          item.qtd_procedimentos = item.qtd_procedimentos || 0;
          item.qtd_contas_fmt = Number(item.qtd_contas).toLocaleString('pt-BR');
          item.qtd_procedimentos_fmt = Number(item.qtd_procedimentos).toLocaleString('pt-BR');
        }
        if (dominio === 'fisioterapia') {
          item.qtd_contas = item.qtd_contas || 0;
          item.qtd_procedimentos = item.qtd_atendimentos || 0;
          item.qtd_contas_fmt = Number(item.qtd_contas).toLocaleString('pt-BR');
          item.qtd_procedimentos_fmt = Number(item.qtd_procedimentos).toLocaleString('pt-BR');
        }
      });
    ...
    // Configuração dos indicadores disponíveis no endpoint médico.
    ...
    return {
      ...
      obterDadosDashboard: function(req) {
        if (!USAR_MOCK) {
          ...
          return $http.post(ENDPOINT, adaptarRequisicao(req), { timeout: 120000, headers: token ? { Authorization: 'Bearer ' + token } : {} }).then(function(response) {
            return adaptarResposta(response.data);
          }, function(response) {
