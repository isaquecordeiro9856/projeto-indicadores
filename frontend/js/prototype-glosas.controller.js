var app = angular.module('DashboardApp');
  ...
  // ----------------------------------------------------------------------
  app.controller('DashboardController', function($scope, $timeout, $q, ApiService, AuthService) {
    ...
    // ----------------------------------------------------------------------
    var CHAVE_ESTADO = 'glosas_dashboard_estado_v1';
    var CHAVE_COLUNAS = 'glosas_dashboard_colunas_v1';
    var CHAVE_TEMA = 'glosas_dashboard_tema_v1';

    // Listas canônicas de validação (fonte única — evita divergência)
    var DIMENSOES_VALIDAS = ['convenio', 'estabelecimento', 'mes', 'setor', 'tipo_convenio', 'tipo_protocolo'];
    var INDICADORES_VALIDOS = ['todos', 'valor_faturado', 'valor_recebido', 'valor_a_receber', 'valor_glosado', 'valor_glosa_aceita', 'valor_reapresentado', 'valor_adicional', 'valor_retorno', 'pct_recebido', 'pct_glosado', 'pct_glosa_aceita', 'pct_adicional'];
    ...
    var PALETA_ESCURA_INDICADORES = {
      ...
      valor_a_receber: '#67d3fb',
      valor_glosado: '#f87171',
      valor_glosa_aceita: '#fca5a5',
      valor_reapresentado: '#fbbf24',
      valor_adicional: '#c4b5fd',
      valor_retorno: '#5eead4',
      pct_recebido: '#4ade80',
      pct_glosado: '#f87171',
      pct_glosa_aceita: '#fca5a5',
      pct_adicional: '#c4b5fd'
    };
    ...
    $scope.definicaoColunas = [
      ...
      { key: 'valor_a_receber',     rotulo: 'Val. a Receber',     badgeClass: 'badge-val-a-receber' },
      { key: 'valor_glosado',       rotulo: 'Val. Glosado',       badgeClass: 'badge-val-glosado',       inverterDelta: true },
      { key: 'valor_glosa_aceita',  rotulo: 'Glosa Aceita',       badgeClass: 'badge-val-glosa-aceita',  inverterDelta: true },
      { key: 'valor_reapresentado', rotulo: 'Val. Reapresentado', badgeClass: 'badge-val-reapresentado' },
      { key: 'valor_adicional',     rotulo: 'Val. Adicional',     badgeClass: 'badge-val-adicional' },
      { key: 'valor_retorno',       rotulo: 'Val. Retorno',       badgeClass: 'badge-val-retorno' },
      { key: 'pct_recebido',        rotulo: '% Recebido',         badgeClass: 'badge-pct-recebido' },
      { key: 'pct_glosado',         rotulo: '% Glosado',          badgeClass: 'badge-pct-glosado',       inverterDelta: true },
      { key: 'pct_glosa_aceita',    rotulo: '% Glosa Aceita',     badgeClass: 'badge-pct-glosa-aceita',  inverterDelta: true },
      { key: 'pct_adicional',       rotulo: '% Adicional',        badgeClass: 'badge-pct-adicional' }
    ];
    ...
    $scope.exportarPNG = function() {
      ...
      try {
        ...
        link.download = 'Tasy_Glosas_' + $scope.config.dimensao + '_' + new Date().toISOString().substring(0, 10) + '.png';
    ...
    function calcularTotaisTabela() {
      ...
      if (!res.modoTodos) {
        ...
        var chavesValor = ['valor_faturado', 'valor_recebido', 'valor_a_receber', 'valor_glosado', 'valor_glosa_aceita', 'valor_reapresentado', 'valor_adicional', 'valor_retorno'];
        ...
        t.pct_recebido = fat > 0 ? (t.valor_recebido / fat) * 100 : 0;
        t.pct_glosado = fat > 0 ? (t.valor_glosado / fat) * 100 : 0;
        t.pct_glosa_aceita = fat > 0 ? (t.valor_glosa_aceita / fat) * 100 : 0;
        t.pct_adicional = fat > 0 ? (t.valor_adicional / fat) * 100 : 0;
    ...
    // ======================================================================
    var CHAVE_METAS = 'glosas_dashboard_metas_v1';
    var CHAVE_ANOMALIA = 'glosas_dashboard_anomalia_v1';
    var CHAVE_VIEWS = 'glosas_dashboard_views_v1';
    var CHAVE_CACHE = 'glosas_dashboard_cache_v1';

    var METAS_PADRAO = {
      pct_glosado:      { valor: 4,  tipo: 'max' },
      pct_glosa_aceita: { valor: 1.5, tipo: 'max' },
      pct_recebido:     { valor: 95, tipo: 'min' },
      pct_adicional:    { valor: 2,  tipo: 'min' }
    };

    $scope.metas = angular.copy(METAS_PADRAO);
    $scope.painelMetasAberto = false;
    $scope.conformidadeGlosa = null;
    ...
    function calcularConformidadeGlosa() {
      var res = $scope.respostaBackend;
      if (!res || !res.modoTodos || !res.dados || !res.dados.length) { $scope.conformidadeGlosa = null; return; }
      var limite = parseFloat($scope.metas.pct_glosado.valor) || 0;
      var dentro = res.dados.filter(function(it) { return (it.pct_glosado || 0) <= limite; }).length;
      $scope.conformidadeGlosa = dentro;
    }

    // Classificação de glosa agora respeita a meta configurada
    ...
    $scope.glosaRateClass = function(item) {
      ...
      var limBaixa = parseFloat($scope.metas.pct_glosado.valor) || 4;
    ...
    $scope.painelVarMetrica = { metrica: 'pct_glosado' };
    ...
    $scope.dicionarioIndicadores = {
      ...
      valor_recebido:       { nome: 'Valor Recebido',       descricao: 'Valores efetivamente pagos ou creditados pelas operadoras no período (R$).' },
      valor_a_receber:      { nome: 'Valor a Receber',      descricao: '= Faturado − Recebido − Glosa Aceita. Pendências ainda em aberto (R$). Maior é melhor.' },
      ...
      valor_glosa_aceita:   { nome: 'Valor Glosa Aceita',   descricao: 'Parcela da glosa reconhecida como procedente após auditoria (R$). Menor é melhor.' },
      valor_reapresentado:  { nome: 'Valor Reapresentado',  descricao: 'Glosas contestadas por recurso e reapresentadas à operadora (R$).' },
      valor_adicional:      { nome: 'Valor Adicional',      descricao: 'Acréscimos obtidos em revisão de conta — itens não faturados, UTI etc. (R$).' },
      valor_retorno:        { nome: 'Valor Retorno',        descricao: 'Valores recuperados de glosas aceitas, via retorno/reversão (R$). Maior é melhor.' },
      pct_recebido:         { nome: '% Recebido',           descricao: '= Recebido ÷ Faturado × 100. Adimplência das operadoras (%). Maior é melhor.' },
      pct_glosado:          { nome: '% Glosado',            descricao: '= Glosado ÷ Faturado × 100. Principal termômetro de perda de receita (%). Menor é melhor.' },
      pct_glosa_aceita:     { nome: '% Glosa Aceita',       descricao: '= Glosa Aceita ÷ Faturado × 100. Parcela da glosa considerada legítima (%). Menor é melhor.' },
      pct_adicional:        { nome: '% Adicional',          descricao: '= Adicional ÷ Faturado × 100. Receita recuperada em revisões (%). Maior é melhor.' }
    };
    ...
    function calcularKPIs(res) {
      ...
      if (res.modoTodos) {
        ...
        let totalAReceber = fonteKpis.reduce((acc, item) => acc + (item.valor_a_receber || 0), 0);
        let totalGlosado = fonteKpis.reduce((acc, item) => acc + (item.valor_glosado || 0), 0);
        let totalGlosaAceita = fonteKpis.reduce((acc, item) => acc + (item.valor_glosa_aceita || 0), 0);
        let totalReapresentado = fonteKpis.reduce((acc, item) => acc + (item.valor_reapresentado || 0), 0);
        let totalAdicional = fonteKpis.reduce((acc, item) => acc + (item.valor_adicional || 0), 0);
        let totalRetorno = fonteKpis.reduce((acc, item) => acc + (item.valor_retorno || 0), 0);

        let taxaGlosa = totalFaturado > 0 ? (totalGlosado / totalFaturado) * 100 : 0;
        let taxaGlosaAceita = totalFaturado > 0 ? (totalGlosaAceita / totalFaturado) * 100 : 0;
        let taxaRecebimento = totalFaturado > 0 ? (totalRecebido / totalFaturado) * 100 : 0;
        let taxaAdicional = totalFaturado > 0 ? (totalAdicional / totalFaturado) * 100 : 0;
        ...
        let aRecAnt = somarCampo('valor_a_receber_ant');
        ...
        let varARec = variacaoPct(totalAReceber, aRecAnt);
        ...
        $scope.kpis = {
          ...
          aReceberFormatado: totalAReceber.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }),
          aReceberAnteriorFormatado: aRecAnt.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }),
          varAReceber: varARec,
          ...
          nums: {
            pct_glosado: taxaGlosa,
            pct_glosa_aceita: taxaGlosaAceita,
            pct_recebido: taxaRecebimento,
            pct_adicional: taxaAdicional
          }
        };
    ...
    function renderizarTendencia() {
      ...
      function montarSerie(cfgKey, cor, rotulo, fonte, tracejada) {
        ...
        var isPct = ind ? ind.isPercentual : false;
        return {
          label: rotulo,
      ...
      if (cacheTendAtual.modoTodos) {
        ...
        else if ($scope.filtroSerieTendencia === 'percentuais') chaves = Object.keys(cfgMap).filter(k => cfgMap[k].isPercentual);
        else if ($scope.filtroSerieTendencia === 'glosas_vs_recuperacao') chaves = ['valor_glosado', 'valor_glosa_aceita', 'valor_reapresentado', 'valor_retorno'];
        ...
        datasets.push({
          ...
          isPercentual: cacheTendAtual.isPercentual,
          yAxisID: cacheTendAtual.isPercentual ? 'yPct' : 'yFinanc'
        });
        ...
        if (dadosAntOrdenados) {
          datasets.push({
            ...
            isPercentual: cacheTendAnterior.isPercentual,
            yAxisID: cacheTendAnterior.isPercentual ? 'yPct' : 'yFinanc'
          });
        }
      }

      var temMoeda = datasets.some(ds => ds.yAxisID === 'yFinanc');
      var temPct = datasets.some(ds => ds.yAxisID === 'yPct');
      ...
      if (temPct) {
        scales.yPct = {
          ...
          ticks: { font: { size: 10 }, color: cg.tick, callback: v => v + '%' }
        };
      }
            ...
            y: {
              ...
              ticks: {
                ...
                callback: function(v) {
                  ...
                  return isPct ? v + '%' : fmtValorCurto(v, true, false);
                }
              }
            }
          }
    ...
    function montarConteudoCsv(res) {
      ...
      if (res.modoTodos) {
        csvRows.push([
          res.nomeDimensao, 
          'Valor_Faturado_R$', 'Valor_Recebido_R$', 'Valor_a_Receber_R$',
          'Valor_Glosado_R$', 'Valor_Glosa_Aceita_R$', 'Valor_Reapresentado_R$',
          'Valor_Adicional_R$', 'Valor_Retorno_R$', 
        ...
        res.dados.forEach(function(d) {
          csvRows.push([
            ...
            numCsv(d.valor_a_receber),
            numCsv(d.valor_glosado),
            numCsv(d.valor_glosa_aceita),
            numCsv(d.valor_reapresentado),
            numCsv(d.valor_adicional),
            numCsv(d.valor_retorno),
            numCsv(d.pct_recebido),
            numCsv(d.pct_glosado),
            numCsv(d.pct_glosa_aceita),
            numCsv(d.pct_adicional)
          ].join(';'));
        });
    ...
    function baixarCsv(conteudo, sufixoArquivo) {
      ...
      link.setAttribute('download', `Tasy_Glosas_${$scope.config.dimensao}${sufixoArquivo}_${new Date().toISOString().substring(0,10)}.csv`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    ...
    $scope.exportarExcel = function() {
      ...
      html += `<h3>TASY Analytics — Glosas por Convênio (${esc(res.nomeDimensao)}) (${esc($scope.infoPeriodo.rotuloAtual)})</h3>`;
      html += '<table border="1"><thead><tr style="background-color:#0f4c81;color:#ffffff;font-weight:bold;">';

      if (res.modoTodos) {
        html += `<th>${esc(res.nomeDimensao)}</th>`;
        ['Val. Faturado','Val. Recebido','Val. a Receber','Val. Glosado','Glosa Aceita','Reapresentado','Adicional','Retorno','% Recebido','% Glosado','% Glosa Aceita','% Adicional'].forEach(function(h) {
          html += `<th>${h}</th>`;
        });
        html += '</tr></thead><tbody>';
        lista.forEach(function(d) {
          html += `<tr><td>${esc(d.label)}</td>` +
            ...
            `<td>${(d.valor_a_receber || 0).toFixed(2).replace('.', ',')}</td>` +
            `<td>${(d.valor_glosado || 0).toFixed(2).replace('.', ',')}</td>` +
            `<td>${(d.valor_glosa_aceita || 0).toFixed(2).replace('.', ',')}</td>` +
            `<td>${(d.valor_reapresentado || 0).toFixed(2).replace('.', ',')}</td>` +
            `<td>${(d.valor_adicional || 0).toFixed(2).replace('.', ',')}</td>` +
            `<td>${(d.valor_retorno || 0).toFixed(2).replace('.', ',')}</td>` +
            `<td>${(d.pct_recebido || 0).toFixed(2).replace('.', ',')}</td>` +
            `<td>${(d.pct_glosado || 0).toFixed(2).replace('.', ',')}</td>` +
            `<td>${(d.pct_glosa_aceita || 0).toFixed(2).replace('.', ',')}</td>` +
            `<td>${(d.pct_adicional || 0).toFixed(2).replace('.', ',')}</td></tr>`;
        });
      } else {
      ...
      let link = document.createElement('a');
      link.setAttribute('href', url);
      link.setAttribute('download', `Tasy_Glosas_${$scope.config.dimensao}_${new Date().toISOString().substring(0,10)}.xls`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      setTimeout(function() { URL.revokeObjectURL(url); }, 1000);
    ...
    $scope.copiarTabela = function() {
      ...
      if (res.modoTodos) {
        ...
        textRows.push([
          res.nomeDimensao, 'Faturado', 'Recebido', 'A Receber',
          'Glosado', 'Glosa Aceita', 'Reapresentado', 'Adicional', 'Retorno',
          '% Recebido', '% Glosado', '% Glosa Aceita', '% Adicional'
        ].join('\t'));
        res.dados.forEach(d => {
          textRows.push([
            d.label, d.valor_faturado_fmt, d.valor_recebido_fmt, d.valor_a_receber_fmt,
            d.valor_glosado_fmt, d.valor_glosa_aceita_fmt, d.valor_reapresentado_fmt,
            d.valor_adicional_fmt, d.valor_retorno_fmt,
            pctTxt(d.pct_recebido), pctTxt(d.pct_glosado),
            pctTxt(d.pct_glosa_aceita), pctTxt(d.pct_adicional)
          ].join('\t'));
        });
      } else {
    ...
    function renderizarGraficos(dadosResp) {
      ...
      if (dadosResp.modoTodos) {
        ...
        } else if ($scope.filtroSerieGrafico === 'percentuais') {
          chavesFiltradas = chavesFiltradas.filter(k => configMap[k].isPercentual);
        } else if ($scope.filtroSerieGrafico === 'glosas_vs_recuperacao') {
          chavesFiltradas = ['valor_glosado', 'valor_glosa_aceita', 'valor_reapresentado', 'valor_retorno'];
        }
        ...
        chavesFiltradas.forEach(key => {
          ...
          datasets1.push({
            ...
            borderRadius: 4,
            isMoeda: ind.isMoeda,
            isPercentual: ind.isPercentual,
            xAxisID: isHorizontal ? (ind.isPercentual ? 'xPct' : 'xFinanc') : undefined,
            yAxisID: !isHorizontal ? (ind.isPercentual ? 'yPct' : 'yFinanc') : undefined
          });
        });

        let temMoeda = chavesFiltradas.some(k => configMap[k].isMoeda);
        let temPct = chavesFiltradas.some(k => configMap[k].isPercentual);
        ...
        if (isHorizontal) {
          ...
          if (temPct) {
            scalesConfig.xPct = {
              type: 'linear',
              position: 'top',
              title: { display: true, text: 'Percentuais (%)', font: { size: 10, weight: '700' }, color: '#059669' },
              grid: { drawOnChartArea: !temMoeda },
              ticks: {
                font: { size: 10.5 },
                color: cg.tick,
          ...
          if (temPct) {
            scalesConfig.yPct = {
              ...
              title: { display: true, text: 'Percentuais (%)', font: { size: 10, weight: '700' } },
              grid: { drawOnChartArea: !temMoeda },
              ticks: {
                callback: function(v) { return v + '%'; }
              }
