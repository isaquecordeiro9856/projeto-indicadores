...
const DIMENSOES_CC = {
  ...
  porte: {
    query: `
      SELECT
        COALESCE(p.ie_porte_cirurgia, 'Não Informado') as label,
        COUNT(DISTINCT pp.nr_atendimento) as qtd_contas,
        COUNT(*) as qtd_procedimentos,
        COALESCE(SUM(pp.vl_procedimento), 0) as valor_produzido
      FROM ods.procedimento_paciente pp
...
// específica da tabela (ex.: clicar num médico traz só a evolução dele).
const COLUNA_FILTRO_MES = {
  ...
  porte: "COALESCE(p.ie_porte_cirurgia, 'Não Informado')"
};
...
function buscarCentroCirurgico(req, res) {
  try {
    ...
    pool.query(consulta.query, consulta.parametros).then(function(result) {
      ...
      if (indicador && indicador !== 'todos') {
        var cfg = { qtd_contas: { nome: 'Atendimentos' }, qtd_procedimentos: { nome: 'Procedimentos' }, valor_produzido: { nome: 'Valor Produzido', isMoeda: true } };
        if (!cfg[indicador]) {
      ...
      dados.sort(function(a, b) { return ordem === 'asc' ? a.qtd_procedimentos - b.qtd_procedimentos : b.qtd_procedimentos - a.qtd_procedimentos; });
      let totalBase = dados.length;
      dados = dados.slice(0, limite);
      ...
      logger.error({ err: error }, 'Erro na query de centro cirúrgico');
      res.status(500).json({ mensagem: 'Erro ao consultar dados de centro cirúrgico: ' + error.message });
    });
  } catch (error) {
    logger.error({ err: error }, 'Erro no controller de centro cirúrgico');
    res.status(500).json({ mensagem: 'Erro interno do servidor' });
  }
}

module.exports = { buscarCentroCirurgico };
