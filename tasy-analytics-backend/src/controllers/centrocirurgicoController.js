...
function buscarCentroCirurgico(req, res) {
  try {
    ...
    const config = DIMENSOES_CC[dimensao];
    ...
    pool.query(consulta.query, consulta.parametros).then(function(result) {
      ...
      console.error('Erro na query de centro cirúrgico:', error.message);
      res.status(500).json({ mensagem: 'Erro ao consultar dados de centro cirúrgico: ' + error.message });
    });
  } catch (error) {
    console.error('Erro no controller de centro cirúrgico:', error);
